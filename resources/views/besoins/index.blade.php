<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="mb-0" style="font-size:1.5rem;">Besoins d'aide</h2>
            <p class="header-sub mb-0">Aidez ceux qui en ont besoin près de chez vous.</p>
        </div>
    ...
    <div class="ls-count">
        <strong>{{ $besoins->total() }}</strong> besoin(s) déclaré(s)
    </div>
    ...
    @if($besoins->isEmpty())
        <div class="ls-empty-card">
            ...
            <h5 class="ls-empty-title">Aucun besoin déclaré</h5>
            <p class="ls-empty-desc">Revenez bientôt, de nouvelles demandes apparaîtront prochainement.</p>
        </div>
    @else
        <div class="ls-grid">
                    ...
                    <div class="ls-card-body d-flex flex-column">
                        ...
                        <div class="ls-card-top">
                            <div class="ls-card-avatar" style="background: var(--cl-red-glow); color: var(--cl-red);">
                                @if($besoin->categorie === 'alimentaire' || $besoin->categorie === 'alimentation')
                                ...
                                @elseif($besoin->categorie === 'medical' || $besoin->categorie === 'sante')
                            ...
                            <div class="ls-card-info">
                                <h6 class="ls-card-title">{{ $besoin->nom }}</h6>
                            ...
                            @if($besoin->urgence === 'critique')
                                <span class="ls-badge ls-badge--red">Critique</span>
                            @elseif($besoin->urgence === 'urgente')
                                <span class="ls-badge ls-badge--orange">Urgente</span>
                            @else
                                <span class="ls-badge ls-badge--muted">Normale</span>
                            @endif
                        </div>
                        ...
                        {{-- Description --}}
                        <p class="ls-card-desc">{{ Str::limit($besoin->description, 100) }}</p>
                        ...
                        <div class="ls-card-actions mt-auto">
                            ...
                            @auth
                                @if(auth()->user()->isAssociation() && $besoin->status === 'en_attente')
                                    <form method="POST" action="{{ route('besoins.prendre_en_charge', $besoin) }}">
                                        @csrf
                                        <button type="submit" class="ls-btn ls-btn--red" onclick="return confirm('Prendre en charge ce besoin ?')">
    ...
    <div class="ls-cta-card mt-4">
        ...
        <h5 class="ls-cta-title">Vous avez besoin d'aide ?</h5>
        <p class="ls-cta-desc">Déclarez votre besoin — sans inscription</p>
...
<style>
    ...
    .ls-assigned-block {
        ...
        font-size: 0.78rem; color: #1A8C38;
    }
    .ls-assigned-block svg { flex-shrink: 0; }
    ...
    .ls-btn svg { flex-shrink: 0; }
    .ls-btn--red { background: var(--cl-red); color: #fff; box-shadow: 0 2px 8px rgba(230,57,70,0.2); }
    .ls-btn--red:hover { background: var(--cl-red-hover); color: #fff; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(230,57,70,0.28) !important; }
    .ls-btn--ghost { background: transparent; color: var(--cl-muted); border: 1.5px solid var(--cl-border) !important; }
    .ls-btn--ghost:hover { color: var(--cl-dark); border-color: var(--cl-muted) !important; background: var(--cl-light); }
    ...
    .ls-cta-btn:hover { background: var(--cl-red-hover); color: #fff; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(230,57,70,0.3); }
    ...
    @media (max-width: 767.98px) {
        ...
        .ls-card-actions { flex-direction: column; align-items: stretch; }
        .ls-card-actions > div:first-child { justify-content: space-between; }
        .ls-btn { justify-content: center; }
        .ls-cta-card { padding: 2rem 1.25rem; border-radius: var(--radius-lg); box-shadow: none; border: none; }
